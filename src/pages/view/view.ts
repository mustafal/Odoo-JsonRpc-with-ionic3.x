...
export class ViewPage {
  ...
  private display(): void {
    ...
    let fields = ["name", "email", "mobile", "phone",
      "title", "street", "street2", "city", "state_id", "function",
      "country_id", "zip", "website", "image"];
    let domain = [
      ["id", "=", this.partnerId]
    ]
    let sort = ""
    ...
    this.odooRpc.searchRead(partner, domain, fields, limit, offset, sort).then((res: any) => {
      ...
      for (let record in data) {
        ...
        this.data.push({
          ...
          website: data[record].website == false ? "N/A" : data[record].website,
          function: data[record].function == false ? "N/A" : data[record].function
        })
      }
    })
  }
